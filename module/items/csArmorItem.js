...
export class CSArmorItem extends CSItem {
  onEquippedChanged(actor, isEquipped) {
    ...
    if (isEquipped) {
      ...
      actor.addModifier(
        ChronicleSystem.modifiersConstants.DAMAGE_TAKEN,
        this._id,
        this.getCSData().rating
      );
    } else {
      actor.removeModifier(
        ChronicleSystem.modifiersConstants.AGILITY,
        this._id
      );
      actor.removeModifier(
  ...
  onObtained(actor) {
    LOGGER.trace(
      `Armor ${this._id} obtained by the actor ${actor.name} | csArmorItem.js`
    );
