...
export class CsCombat extends Combat {
  ...
  async rollInitiative(
    ...
    if (result) {
      ...
      type = SystemUtils.localize(
        CSConstants.InitiativeTypes[result.initiativeType.value]
      );
    }
      ...
      let messageData = foundry.utils.mergeObject(
        {
          ...
          flavor: game.i18n.format("CS.chatMessages.initiativeRolls", {
            name: combatant.name,
            type: type,
          }),
  ...
  async _showInitiativeSelector() {
    ...
    const html = await foundry.applications.handlebars.renderTemplate(
      template,
      { choices: CSConstants.InitiativeTypes }
    );
